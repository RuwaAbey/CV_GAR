...
class Temporal_Attention(nn.Module):
    def __init__(self, dim, heads=3, dropout_rate=0.1, relative_pos=True, drop_connectivity=False):
        super().__init__()
        self.heads = heads
    ...
    def forward(self, x):
        N, C, T, V = x.size()
        if self.relative and not hasattr(self, 'key_rel_embed'):
            self.key_rel_embed = nn.Parameter(torch.randn(2 * T - 1, self.dim // self.heads, requires_grad=True))
    ...
    def relative_logits(self, q):
        B, Nh, dk, F, T = q.size()
        q = q.permute(0, 1, 3, 4, 2).reshape(B, Nh, F * T, dk // Nh)
        rel_logits = self.relative_logits_1d(q, self.key_rel_embed)
        return rel_logits.view(B, Nh, T, T)
