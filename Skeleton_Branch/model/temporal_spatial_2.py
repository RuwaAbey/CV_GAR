...
class Temporal_Attention(nn.Module):
    ...
    def forward(self,x,T = 1):
        ...
        #Removing the Brnoulli mask
        #mask = torch.bernoulli((0.5) * torch.ones(B * self.heads * T, device=device))
        #mask = mask.reshape(B, self.Nh, T).unsqueeze(2).expand(B, self.heads, T, T)
...
class TCN_STTRANSF_unit(nn.Module):
    def __init__(self, in_channels, out_channels,num_point, heads=3, stride=1, residual=True, dropout=0.1, mask=None, mask_grad=True, group_mask=None):
        super(TCN_STTRANSF_unit, self).__init__()
        # Pass mask to Spatial_Transformer
        self.transf1 = Temporal_Transformer(dim=in_channels, depth=4, heads=heads, mlp_dim=in_channels, dropout=dropout, num_point=num_point, mask=mask,group_mask=group_mask)
        self.transf2 = Spatial_Transformer(dim=in_channels, depth=2, heads=heads, mlp_dim=in_channels, dropout=dropout, num_point=num_point, mask=mask,group_mask=group_mask)
    ...
    def forward(self, x, mask=None,group_mask=None):
        ...
        if mask==None:
            ...
            ts = self.transf2(tx, self.mask,group_mask=group_mask) #change input to x

        else:
            tx = self.transf1(tx, mask,T=T) #change input to x
            ts = self.transf2(tx, mask,group_mask=group_mask) #change input to x
        tx = tx.view(B, T, V, C).permute(0, 3, 1, 2).contiguous()
        ts = ts.view(B, T, V, C).permute(0, 3, 1, 2).contiguous()

        x = self.tcn1(tx) + self.residual(x) + self.tcn1(ts)
        return self.relu(x)
...
class ZiT(nn.Module):
    def __init__(self, in_channels=3, num_person=12, num_point=17, num_head=6, graph=None, graph_args=dict()):
        ...
        self.l2 = TCN_STTRANSF_unit(48, 48, num_point=num_point, heads=num_head, mask=self.A, mask_grad=False)      
        self.l3 = TCN_STTRANSF_unit(48, 48,num_point=num_point, heads=num_head, mask=self.A, mask_grad=False)
        self.l4 = TCN_STTRANSF_unit(48, 96, num_point=num_point, heads=num_head, stride=2, mask=self.A, mask_grad=True)
        self.l5 = TCN_STTRANSF_unit(96, 96,num_point=num_point, heads=num_head, mask=self.A, mask_grad=True)
        self.l6 = TCN_STTRANSF_unit(96, 192, num_point=num_point,heads=num_head, stride=2, mask=self.A, mask_grad=True)
        self.l7 = TCN_STTRANSF_unit(192, 192, num_point=num_point,heads=num_head, mask=self.A, mask_grad=True)
