...
class Temporal_Attention(nn.Module):
    def __init__(self, dim, out_dim, heads=3, dropout=0.1, kernel_size=1, stride=1, num_point=17):
        ...
        self.data_bn = nn.BatchNorm1d(self.dim * num_point)
        ...
        frames = {48: 10, 96: 10, 192: 10}.get(self.dim, 150)
        self.key_rel = nn.Parameter(torch.randn(2 * frames - 1, self.dim // self.heads))
    ...
    def forward(self, x, mask=None, T=1):
        ...
        flat_q, flat_k, flat_v, q, k, v = self.compute_flat_qkv(x, self.dim, self.dv, self.Nh)
        ...
        dropout_mask = torch.bernoulli(torch.full((B * self.Nh * T,), 0.5, device=device)).reshape(B, self.Nh, T, 1)
        weights = weights * dropout_mask.expand(-1, -1, -1, T)
        weights = weights / (weights.sum(dim=3, keepdim=True) + 1e-8)

        attn_out = torch.matmul(weights, flat_v.transpose(2, 3)).reshape(B, self.Nh, 1, T, self.dv // self.Nh)
        attn_out = attn_out.permute(0, 1, 2, 4, 3)
        attn_out = self.combine_heads_2d(attn_out)
        attn_out = self.attn_out(attn_out)

        attn_out = attn_out.reshape(N, V, -1, T).permute(0, 2, 3, 1)
        out = attn_out.permute(0, 2, 3, 1).contiguous().squeeze(1).view(-1, attn_out.size(2), attn_out.size(3))
...
class TCN_TTRANSF_unit(nn.Module):
    def __init__(self, in_channels, out_channels, heads=3, stride=1, residual=True, dropout=0.1, mask=None, mask_grad=True):
        ...
        self.transf1 = Temporal_Transformer(dim=in_channels, depth=1, heads=heads, mlp_dim=in_channels, dropout=dropout)
...
class ZiT(nn.Module):
    def __init__(self, in_channels=3, num_person=5, num_point=17, num_head=6, graph=None, graph_args=dict()):
        ...
        self.l2 = TCN_TTRANSF_unit(48, 48, heads=num_head, mask=self.A, mask_grad=False)      
        self.l3 = TCN_TTRANSF_unit(48, 48, heads=num_head, mask=self.A, mask_grad=False)
        self.l4 = TCN_TTRANSF_unit(48, 96, heads=num_head, stride=2, mask=self.A, mask_grad=True)
        self.l5 = TCN_TTRANSF_unit(96, 96, heads=num_head, mask=self.A, mask_grad=True)
        self.l6 = TCN_TTRANSF_unit(96, 192, heads=num_head, stride=2, mask=self.A, mask_grad=True)
        self.l7 = TCN_TTRANSF_unit(192, 192, heads=num_head, mask=self.A, mask_grad=True)
...
class ZoT(nn.Module):
    ...
    def forward(self, x):
        ...
        group_mask = group_mask.permute(0, 2, 1, 3, 4).contiguous().view(N * T, heads, M, M)
