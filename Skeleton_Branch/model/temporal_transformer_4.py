...
class TCN_TTRANSF_unit(nn.Module):
    def __init__(self, in_channels, out_channels,A ,num_point, heads=3, stride=1, residual=True, dropout=0.1, mask=None, mask_grad=True, group_mask=None):
        ...
        self.tcn1 = unit_tcn(out_channels, out_channels,  kernel_size=1, stride=stride)
        self.gcn1 = unit_gcn(in_channels, out_channels, A)
        self.relu = nn.ReLU()
...
class ZiT(nn.Module):
    def __init__(self, in_channels=3, num_person=12, num_point=17, num_head=6, graph=None, graph_args=dict()):
        ...
        self.l2 = TCN_TTRANSF_unit(48, 48, self.graph.A, num_point=num_point, heads=num_head, mask=self.A, mask_grad=False)      
        self.l3 = TCN_TTRANSF_unit(48, 48, self.graph.A, num_point=num_point, heads=num_head, mask=self.A, mask_grad=False)
        self.l4 = TCN_TTRANSF_unit(48, 96, self.graph.A, num_point=num_point, heads=num_head, stride=2, mask=self.A, mask_grad=True)
        self.l5 = TCN_TTRANSF_unit(96, 96, self.graph.A, num_point=num_point, heads=num_head, mask=self.A, mask_grad=True)
        self.l6 = TCN_TTRANSF_unit(96, 192, self.graph.A, num_point=num_point,heads=num_head, stride=2, mask=self.A, mask_grad=True)
        self.l7 = TCN_TTRANSF_unit(192, 192, self.graph.A, num_point=num_point,heads=num_head, mask=self.A, mask_grad=True)
