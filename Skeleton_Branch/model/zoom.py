...
class Temporal_Attention(nn.Module):
    def __init__(self, in_channels, out_channels, dv_factor, dk_factor, Nh, n,
                 relative, kernel_size_temporal, stride, more_channels, num_point):
        super().__init__()
        ...
        self.key_rel = nn.Parameter(torch.randn(2 * frames - 1, self.dk // Nh))
    ...
    def forward(self, x):
        ...
        mask = torch.bernoulli(torch.full((B * self.Nh * T,), 0.5, device=device)).reshape(B, self.Nh, T, 1)
    ...
    def compute_flat_qkv(self, x):
        ...
        flat_q = q.reshape(N, self.Nh, dkh, V1 * T1)
        flat_k = k.reshape(N, self.Nh, dkh, V1 * T1)
        flat_v = v.reshape(N, self.Nh, self.dv // self.Nh, V1 * T1)

        return flat_q, flat_k, flat_v

    def split_heads_2d(self, x):
        B, channels, F, V = x.shape
        return x.reshape(B, self.Nh, channels // self.Nh, F, V)
