#updated the handling of T from the spatial transformer
import math
...
class Spatial_Attention(nn.Module):
    def __init__(self, dim, out_dim, heads=3, dropout=0.1, num_point=17):
        ...
        self.do1 = nn.Dropout(dropout)
        # Add dropout for relative logits
    ...
    def forward(self, x, mask=None, T=1):
        ...
        # Apply dropout to rel_logits using the module's dropout layer
        rel_logits = self.rel_dropout(rel_logits)
        ...
        if dim == mlp_dim:
            for _ in range(depth):
                self.layers.append(nn.ModuleList([
                    Residual(Spatial_Attention(dim, mlp_dim, heads=heads, dropout=dropout, num_point=num_point)),
                    Residual(LayerNormalize(mlp_dim, MLP_Block(mlp_dim, mlp_dim*2, dropout=dropout)))
                ]))
        else:
            for _ in range(depth):
                self.layers.append(nn.ModuleList([
                    Spatial_Attention(dim, mlp_dim, heads=heads, dropout=dropout, num_point=num_point),
                    Residual(LayerNormalize(mlp_dim, MLP_Block(mlp_dim, mlp_dim*2, dropout=dropout)))
                ]))
    def forward(self, x, mask=None, T=1):
...
class Temporal_Attention(nn.Module):
    def __init__(self, dim, mlp_dim,heads=3, dropout_rate=0.1, relative_pos=True, drop_connectivity=False,max_T=100):
        super().__init__()
        self.heads = heads
        ...
        self.attention_output = nn.Conv2d(dim, mlp_dim, kernel_size=1)
    ...
    def forward(self, x,T=1):
        ...
        x = x.view(N,C,T,V).permute(0, 3, 1, 2).reshape(N * V, C, 1, T)  # (B, Nh, dk, T, V)
        ...
        if self.relative:
            ...
            rel_logits = self.temporal_relative_logits(q)
        ...
        attn_out = attn_out.view(N*T, V, C)
    ...
    def temporal_relative_logits(self, q):
        B, Nh, dk, _, T = q.size()
        q = q.permute(0, 1, 3, 4, 2)
        q = q.reshape(B, Nh, T, dk)
        rel_logits = self.temporal_relative_logits_1d(q, self.rel_k)

        return rel_logits

    def temporal_relative_logits_1d(self, q, rel_k):
        rel_logits = torch.einsum('bhld,md->bhlm', q, rel_k)
        rel_logits = self.temporal_rel_to_abs(rel_logits)
        B, Nh, L, L = rel_logits.size()
        return rel_logits

    def temporal_rel_to_abs(self, x):
        B, Nh, L, _ = x.size()
...
class Temporal_Transformer(nn.Module):
    def __init__(self, dim, mlp_dim, depth, heads, dropout):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Residual(Temporal_Attention(dim, mlp_dim,heads=heads, dropout_rate=dropout)),
...
class all_TCN_STRANSF_unit(nn.Module):
    def __init__(self, in_channels, out_channels, num_point, heads=3, stride=1, residual=True, dropout=0.1, mask=None, mask_grad=True):
        super().__init__()
        self.transf1 = Spatial_Transformer(dim=in_channels, depth=1, heads=heads, mlp_dim=in_channels, dropout=dropout, num_point=num_point)
        self.transf2 = Temporal_Transformer(dim=in_channels, mlp_dim=in_channels, depth=1, heads=heads, dropout=dropout)
        self.tcn1 = unit_tcn_m(in_channels, out_channels, stride=stride)
        self.relu = nn.ReLU()
        self.out_channels = out_channels
        
        # Convolutional fusion layer: 1x1 conv to fuse 2*in_channels to out_channels
        self.fusion_conv = nn.Conv2d(in_channels * 2, out_channels, kernel_size=(1, 1), stride=(1, 1), bias=True)
        # Initialize fusion_conv weights and bias
        nn.init.kaiming_normal_(self.fusion_conv.weight, mode='fan_out')
    ...
    def forward(self, x, mask=None, T=1):
        B, C, T_, V = x.size()
        # Derive T from input tensor
        T = T_
        # Spatial and Temporal Transformer processing
        tx = x.permute(0, 2, 3, 1).contiguous().view(B * T, V, C)
        if mask is None and hasattr(self, 'mask'):
            tx_s = self.transf1(tx, self.mask, T=T)
            tx_t = self.transf2(tx, T=T)
        else:
            tx_s = self.transf1(tx, mask, T=T)
            tx_t = self.transf2(tx, T=T)
        
        # Reshape back to (B, C, T, V)
        tx_s = tx_s.view(B, T, V, C).permute(0, 3, 1, 2).contiguous()  # (B, C, T, V)
        tx_t = tx_t.view(B, T, V, C).permute(0, 3, 1, 2).contiguous()  # (B, C, T, V)
        
        # Apply TCN to both spatial and temporal outputs
        tx_s_tcn = self.tcn1(tx_s)  # (B, out_channels, T', V)
        tx_t_tcn = self.tcn1(tx_t)  # (B, out_channels, T', V)
        
        # Stack the feature maps along the channel dimension
        stacked = torch.cat([tx_s_tcn, tx_t_tcn], dim=1)  # (B, 2*out_channels, T', V)
        
        # Apply 1x1 convolution for fusion
        fused = self.fusion_conv(stacked)  # (B, out_channels, T', V)
        
        # Add residual connection
        x = fused + self.residual(x)
        
        return self.relu(x)

class all_ZiT(nn.Module):
    def __init__(self, in_channels=3, num_person=12, num_point=17, num_head=6, graph=None, graph_args=dict()):
        super(all_ZiT, self).__init__()
        ...
        #self.A = self.graph.A[0] + self.graph.A[1] + self.graph.A[2]
        ...
        self.l2 = all_TCN_STRANSF_unit(48, 48, num_point=num_point, heads=num_head, mask=self.A, mask_grad=False)
        self.l3 = all_TCN_STRANSF_unit(48, 48, num_point=num_point, heads=num_head, mask=self.A, mask_grad=False)
        self.l4 = all_TCN_STRANSF_unit(48, 96, num_point=num_point, heads=num_head, stride=2, mask=self.A, mask_grad=True)
        self.l5 = all_TCN_STRANSF_unit(96, 96, num_point=num_point, heads=num_head, mask=self.A, mask_grad=True)
        self.l6 = all_TCN_STRANSF_unit(96, 192, num_point=num_point, heads=num_head, stride=2, mask=self.A, mask_grad=True)
        self.l7 = all_TCN_STRANSF_unit(192, 192, num_point=num_point, heads=num_head, mask=self.A, mask_grad=True)

    def forward(self, x):
        # x: (N, C, T, V, M), where M is num_person
        N, C, T, V, M = x.size()
        # Batch normalization
        ...
        x = self.l1(x)  # Assume TCN_GCN_unit returns (N*M, C', T', V)
        x = self.l2(x)
        x = self.l3(x)
        x = self.l4(x)  # Temporal dimension reduced (stride=2)
        x = self.l5(x)
        x = self.l6(x)  # Temporal dimension reduced (stride=2)
        x = self.l7(x)
        # Reshape output
        B, C_, T_, V_ = x.size()
...
class all_ZoT(nn.Module):
    def __init__(self, num_class=15, num_head=6,num_person=2):
        ...
        self.l1 = all_TCN_STRANSF_unit(192, 276,num_point=num_person, heads=num_head)       # 192 276
        self.l2 = all_TCN_STRANSF_unit(276, 276,num_point=num_person, heads=num_head)
    ...
    def forward(self, x):
        ...
        mask = mask.permute(0, 2, 1, 3, 4).contiguous().view(N*T, C, M, M2)
...
class Model(nn.Module):
    def __init__(self, num_class=60, in_channels=3, num_person=2, num_point=17, num_head=6, graph=None, graph_args=dict()):
        ...
        self.body_transf = all_ZiT(in_channels=in_channels, num_person=num_person, num_point=num_point, num_head=num_head, graph=graph, graph_args=graph_args)
        self.group_transf = all_ZoT(num_class=num_class, num_head=num_head,num_person=num_person)
