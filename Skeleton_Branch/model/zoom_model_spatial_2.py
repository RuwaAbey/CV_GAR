...
class Attention(nn.Module):
    def __init__(self, dim, out_dim, A, heads=3, dropout=0.1, num_point=17):
        super().__init__()
        self.heads = heads
        self.scale = dim ** -0.5
        ...
        self.mask = nn.Parameter(torch.ones(num_point, num_point), requires_grad=True)
        #self.mask = nn.Parameter(torch.from_numpy(A.astype(np.float32)))
    ...
    def forward(self, x, mask=None, T=1):
        ...
        A = self.A * self.mask
        A = A.unsqueeze(0).unsqueeze(0).expand(b, h, n, n)
        dots = dots + A
        if mask is not None:
    ...
    def relative_logits(self, q, T=1):
        ...
        B = B_T // T
        q = q.view(B, T, Nh, V, dk).permute(0, 2, 4, 1, 3)
        ...
        rel_logits = self.relative_logits_1d(q, q_first, self.key_rel.to(q.device), param_diagonal, T, V, Nh)
        return rel_logits

    def relative_logits_1d(self, q, q_first, rel_k, param_diagonal, T, V, Nh):
        rel_logits = torch.matmul(q_first, rel_k.transpose(-2, -1))
        rel_logits = torch.matmul(q_first, rel_k)
        rel_logits_diagonal = torch.matmul(q, param_diagonal)
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim, dropout=0.1, num_point=17, A=None):
        super().__init__()
        self.layers = nn.ModuleList([])
        if dim == mlp_dim:
            for _ in range(depth):
                self.layers.append(nn.ModuleList([
                    Residual(Attention(dim, mlp_dim, A=A, heads=heads, dropout=dropout, num_point=num_point)),
                    Residual(LayerNormalize(mlp_dim, MLP_Block(mlp_dim, mlp_dim*2, dropout=dropout)))
                ]))
        else:
            for _ in range(depth):
                self.layers.append(nn.ModuleList([
                    Attention(dim, mlp_dim, A=A, heads=heads, dropout=dropout, num_point=num_point),
                    Residual(LayerNormalize(mlp_dim, MLP_Block(mlp_dim, mlp_dim*2, dropout=dropout)))
                ]))
    def forward(self, x, mask=None, T=1):
...
class TCN_STRANSF_unit(nn.Module):
    def __init__(self, in_channels, out_channels, num_point, heads=3, stride=1, residual=True, dropout=0.1, A=None):
        super().__init__()
        self.transf1 = Transformer(dim=in_channels, depth=1, heads=heads, mlp_dim=in_channels, dropout=dropout, num_point=num_point, A=A)
...
class ZiT(nn.Module):
    def __init__(self, in_channels=3, num_person=12, num_point=17, num_head=6, graph=None, graph_args=dict()):
        ...
        self.l2 = TCN_STRANSF_unit(48, 48, num_point=num_point, heads=num_head, A=self.A)
        self.l3 = TCN_STRANSF_unit(48, 48, num_point=num_point, heads=num_head, A=self.A)
        self.l4 = TCN_STRANSF_unit(48, 96, num_point=num_point, heads=num_head, stride=2, A=self.A)
        self.l5 = TCN_STRANSF_unit(96, 96, num_point=num_point, heads=num_head, A=self.A)
        self.l6 = TCN_STRANSF_unit(96, 192, num_point=num_point, heads=num_head, stride=2, A=self.A)
        self.l7 = TCN_STRANSF_unit(192, 192, num_point=num_point, heads=num_head, A=self.A)
