...
class Temporal_Attention(nn.Module):
    def __init__(self, dim, out_dim, heads=3, dropout=0.1, relative_pos=True, max_T=100):
        super().__init__()
        self.heads = heads
    ...
    def forward(self, x):
        ...
        # Reshape x to (N*V*T, C) for to_qkv
        x = x.permute(0, 3, 2, 1).reshape(N * V * T, C)  # (N*V*T, C)

        # Apply to_qkv: (N*V*T, C) -> (N*V*T, C*3)
        qkv = self.to_qkv(x)  # (N*V*T, C*3)

        # Reshape to (N*V, h, C/h, T) for q, k, v
        qkv = qkv.reshape(N * V, T, h, 3 * C // h).permute(0, 2, 3, 1)  # (N*V, h, 3*C/h, T)
        q, k, v = qkv.chunk(3, dim=2)  # Each: (N*V, h, C/h, T)

        # Add singleton dimension to get (N*V, h, C/h, 1, T)
        q = q.unsqueeze(3)  # (N*V, h, C/h, 1, T)
        k = k.unsqueeze(3)  # (N*V, h, C/h, 1, T)
        v = v.unsqueeze(3)  # (N*V, h, C/h, 1, T)

        # Flatten for attention: remove singleton dimension
        flat_q = q.squeeze(3)  # (N*V, h, C/h, T)
        flat_k = k.squeeze(3)  # (N*V, h, C/h, T)
        flat_v = v.squeeze(3)  # (N*V, h, C/h, T)

        # Compute attention scores
        dots = torch.einsum('nhdt,nhdj->nhtj', flat_q, flat_k) * self.scale  # (N*V, h, T, T)

        if self.relative:
            self.rel_k = self.key_rel_embed[:(2 * T - 1)]
            rel_logits = self.relative_logits(q)  # (N*V, h, T, T)
            logits_sum = dots + rel_logits
        else:
            logits_sum = dots

        # Apply softmax to get attention weights
        weights = torch.softmax(logits_sum, dim=-1)  # (N*V, h, T, T)

        if self.training:
            mask = torch.bernoulli(0.5 * torch.ones((N * V, h, T, T), device=x.device))
            weights = weights * mask
            weights = weights / (weights.sum(dim=-1, keepdim=True) + 1e-8)

        # Apply attention to values
        out = torch.einsum('nhij,nhdjt->nhdit', weights, flat_v)  # (N*V, h, C/h, 1, T)

        # Reshape back to (N*V*T, C)
        out = out.squeeze(3).permute(0, 3, 1, 2).reshape(N * V * T, C)  # (N*V*T, C)

        # Apply final linear layer and dropout
        out = self.nn1(out)  # (N*V*T, out_dim)
        out = self.do1(out)

        # Reshape to output format (N, out_dim, T, V)
        out = out.reshape(N, V, T, self.out_dim).permute(0, 3, 2, 1)  # (N, out_dim, T, V)
...
class TCN_STRANSF_unit(nn.Module):
    ...
    def forward(self, x):
        ...
        tx = self.transf1(x)  # Remove mask argument
...
class ZiT(nn.Module):
    def __init__(self, in_channels=3, num_person=5, num_point=18, num_head=6, graph=None, graph_args=dict()):
        ...
        self.l4 = TCN_STRANSF_unit(48, 96, heads=num_head, stride=2)
