...
#Adjacency mask is not yet used in the model
...
class Spatial_Attention(nn.Module):
    ...
    def forward(self, x, mask = None):
        ...
        #xa = x.permute(0, 2, 1, 3).reshape(-1, C, 1, V)   #setting dimension for the spatial transformer #(B*T,C,1,V)
        ...
        #out = attn_out.view(-1, x.size(2), x.size(3))    
...
class Spatial_Transformer(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim, dropout,num_point=17):
        super().__init__()
...
class TCN_STRANSF_unit(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask==None:
            tx = self.transf1(tx, self.mask) #change input to x
        else:
            tx = self.transf1(tx, mask) #change input to x
