#Changed the shape of the x, which comes to Spatial_Attention block to xa
#Replaced the spatial attention block architecture with ST-TR Spatial attention block and bernouli mask
#Adjacency mask applied
...
class TCN_STRANSF_unit(nn.Module):
    def __init__(self, in_channels, out_channels,num_point, heads=3, stride=1, residual=True, dropout=0.1, mask=None, mask_grad=True, group_mask=None):
        ...
        self.transf1 = Spatial_Transformer(dim=in_channels, depth=2, heads=heads, mlp_dim=in_channels, dropout=dropout, num_point=num_point, mask=mask,group_mask=group_mask)
...
class AttentionPooling(nn.Module):
    def __init__(self, in_channels, num_person, reduction_ratio=8):
        super(AttentionPooling, self).__init__()
        self.in_channels = in_channels
        self.num_person = num_person
        
        # Reduce channels for attention computation
        reduced_channels = max(in_channels // reduction_ratio, 16)
        
        # Temporal attention: compute base attention scores
        ...
        # Temporal gating: learnable parameters for gated attention
        self.temp_gate = nn.Sequential(
            nn.Conv2d(in_channels, reduced_channels, kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(reduced_channels, 1, kernel_size=1),
            nn.Sigmoid()  # Sigmoid for gating (0 to 1)
        )
        
        # Person attention: compute base attention scores
        ...
        # Person gating: learnable parameters for gated attention
        self.person_gate = nn.Sequential(
            nn.Conv2d(in_channels, reduced_channels, kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(reduced_channels, 1, kernel_size=1),
            nn.Sigmoid()  # Sigmoid for gating (0 to 1)
        ...
        for module in [self.temp_attn, self.temp_gate, self.person_attn, self.person_gate]:
            for layer in module:
    ...
    def forward(self, x):
        ...
        # Temporal attention
        temp_scores = self.temp_attn(x)  # (N, 1, T, M)
        temp_gate = self.temp_gate(x)    # (N, 1, T, M)
        # Combine attention scores with gating (softmax * sigmoid)
        temp_weights = torch.softmax(temp_scores, dim=2) * temp_gate
        temp_weights = temp_weights / (temp_weights.sum(dim=2, keepdim=True) + 1e-8)  # Re-normalize
        # Weighted sum over temporal dimension
        x_temp = (x * temp_weights).sum(dim=2)  # (N, C, M)
        
        # Person attention
        x_temp = x_temp.unsqueeze(2)  # (N, C, 1, M) for convolution
        person_scores = self.person_attn(x_temp)  # (N, 1, 1, M)
        person_gate = self.person_gate(x_temp)    # (N, 1, 1, M)
        # Combine attention scores with gating
        person_weights = torch.softmax(person_scores, dim=3) * person_gate
        person_weights = person_weights / (person_weights.sum(dim=3, keepdim=True) + 1e-8)  # Re-normalize
        # Weighted sum over person dimension
        x_out = (x_temp * person_weights).sum(dim=3).squeeze(2)  # (N, C)
...
class ZoT(nn.Module):
    def __init__(self, num_class=8, num_head=6, num_person=2):
        ...
        self.l1 = TCN_STRANSF_unit(192, 276, num_point=num_person, heads=num_head, mask=None)
        self.l2 = TCN_STRANSF_unit(276, 276, num_point=num_person, heads=num_head, mask=None)

        # Attention pooling with gated attention
        self.attn_pool = AttentionPooling(in_channels=276, num_person=num_person)
    ...
    def forward(self, x):
        ...
        group_mask = mask.softmax(dim=-1)  # [N, heads, T, M, M]
