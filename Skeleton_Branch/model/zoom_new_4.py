...
class AttentionPooling(nn.Module):
    def __init__(self, in_channels, num_person, reduction_ratio=8):
        ...
        self.temp_attn = nn.Sequential(
            ...
            nn.Softmax(dim=2)  # Softmax over temporal dimension
        )
        ...
        self.person_attn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(reduced_channels, 1, kernel_size=1),
            nn.Softmax(dim=3)  # Softmax over person dimension
        )
    ...
    def forward(self, x):
        ...
        # Temporal attention: compute weights for T dimension
        temp_weights = self.temp_attn(x)  # (N, 1, T, M)
        # Weighted sum over temporal dimension
        x_temp = (x * temp_weights).sum(dim=2)  # (N, C, M)
        
        # Person attention: compute weights for M dimension
        person_weights = self.person_attn(x_temp.unsqueeze(2))  # (N, 1, 1, M)
        # Weighted sum over person dimension
        x_out = (x_temp * person_weights.squeeze(2)).sum(dim=2)  # (N, C)
