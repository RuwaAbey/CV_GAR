...
class VertexAttention(nn.Module):
    ...
    def forward(self, x):
        # x: [N, M, C_, T_, V_]
        N, M, C_, T_, V_ = x.size()
        x = x.permute(4, 0, 1, 3, 2)  # [V_, N, M, T_, C_]
        x = x.reshape(V_, -1, C_)  # [V_, N*M*T_, C_]
        x, _ = self.attention(x, x, x)  # Apply attention over V_
        x = self.norm(x)
        x = x.mean(0)  # Aggregate over V_
        return x.view(N, M, T_, C_).permute(0, 3, 2, 1)  # [N, C_, T_, M]
...
class ZiT(nn.Module):
    def __init__(self, in_channels=3, num_person=12, num_point=17, num_head=6, graph=None, graph_args=dict()):
        ...
        # Attention-based vertex aggregation
        self.vertex_agg = VertexAttention(192, num_point)
    ...
    def forward(self, x):
        ...
        B, C_, T_, V_ = x.size()
        x = x.view(N, M, C_, T_, V_)  # Shape: [N, M, C_, T_, V_]
        x = self.vertex_agg(x)  # Shape: [N, C_, T_, M]
