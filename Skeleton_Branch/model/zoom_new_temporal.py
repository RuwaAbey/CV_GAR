...
class Temporal_Attention(nn.Module):
    def __init__(self, dim, out_dim, heads = 3, dropout = 0.25,num_point =17, mask = None,group_mask=None):
        super().__init__()
        self.heads = heads
        ...
        self.qkv_conv = nn.Conv2d(self.dim, 2 * self.dim + self.dim, kernel_size=(1, 1),stride=(1, 1),padding=(0, int((1 - 1) / 2)))
        #here  check about the (stride= (1,stride))
        ...
        self.key_rel = nn.Parameter(
                    torch.randn((2 * frames - 1, self.dim // self.heads), requires_grad=True))

    def forward(self, x, T=1, mask = None,group_mask=None):
        #print(T)
        B, C, T, V = x.size()
        x_sum = x
        ...
        mask = torch.bernoulli((0.5) * torch.ones(B * self.heads * T, device=device))
        mask = mask.reshape(B, self.heads, T).unsqueeze(2).expand(B, self.heads, T, T)
        ...
        attn_out = attn_out.reshape(B, V, -1, T).permute(0, 2, 3, 1)
        attn_out = self.dropout(attn_out)
        result = attn_out+(x_sum if (self.down is None) else self.down(x_sum))
        result = self.bn(result)
        result = self.relu(result) #(N,C,T,V)

        # result = result.permute(0, 2, 3, 1).contiguous() #(N,T,V,C)
        # result = result.squeeze(1).view(-1, attn_out.size(2), attn_out.size(3)) # (B*T, V, C)
...
class Temporal_Transformer(nn.Module):
    ...
    def forward(self, x, mask = None,group_mask=None,T=1):
        for attention, mlp in self.layers:
            x = attention(x,mask=mask,group_mask=group_mask, T=T) # go to attention
...
class TCN_TTRANSF_unit(nn.Module):
    def __init__(self, in_channels, out_channels,num_point, heads=3, stride=1, residual=True, dropout=0.1, mask=None, mask_grad=True,group_mask=None):
        ...
        self.transf1 = Temporal_Transformer(dim=in_channels, depth=1, heads=heads, mlp_dim=in_channels, dropout=dropout,num_point=num_point, mask=mask,group_mask=group_mask)
    ...
    def forward(self, x, mask=None,group_mask=None,T=1):
        ...
        # tx = x.permute(0, 2, 3, 1).contiguous().view(B * T_, V, C)
        if mask==None:
            tx = self.transf1(tx, self.mask,group_mask=group_mask, T=T)
        else:
            tx = self.transf1(tx, mask,group_mask=group_mask,T= T)
        tx = tx.view(B, T_, V, C).permute(0, 3, 1, 2).contiguous()
...
class TCN_STRANSF_unit(nn.Module):
    def __init__(self, in_channels, out_channels,num_point, heads=3, stride=1, residual=True, dropout=0.1, mask=None, mask_grad=True, group_mask=None):
        ...
        self.transf1 = Spatial_Transformer(dim=in_channels, depth=1, heads=heads, mlp_dim=in_channels, dropout=dropout, num_point=num_point, mask=mask,group_mask=group_mask)
        self.tcn1 = unit_tcn_m(in_channels, out_channels, stride=stride)
        self.relu = nn.ReLU()
        self.out_channels = out_channels
        if not residual:
...
class ZiT(nn.Module):
    def __init__(self, in_channels=3, num_person=5, num_point=18, num_head=6, graph=None, graph_args=dict()):
        ...
        self.l2 = TCN_TTRANSF_unit(48, 48,num_point=num_point, heads=num_head, mask=None)
        self.l3 = TCN_TTRANSF_unit(48, 48,num_point=num_point, heads=num_head, mask=None)
        self.l4 = TCN_TTRANSF_unit(48, 96, num_point=num_point,heads=num_head, stride=2, mask=None)
        self.l5 = TCN_TTRANSF_unit(96, 96, num_point=num_point,heads=num_head, mask=None)
        self.l6 = TCN_TTRANSF_unit(96, 192,num_point=num_point, heads=num_head, stride=2, mask=None)
        self.l7 = TCN_TTRANSF_unit(192, 192, num_point=num_point,heads=num_head, mask=None)
...
class ZoT(nn.Module):
    def __init__(self, num_class=15, num_head=6,num_person = 2):
        ...
        self.l1 = TCN_STRANSF_unit(192, 276,num_point=num_person, heads=num_head)       # 192 276
