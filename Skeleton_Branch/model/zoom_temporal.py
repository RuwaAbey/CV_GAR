...
class Temporal_Attention(nn.Module):
    def __init__(self, dim, out_dim, heads = 3 , dropout = 0.1,kernel_size=1,stride=1,num_point =17):
        ...
        self.data_bn = nn.BatchNorm1d(dim * num_point)
        self.relu = nn.ReLU(inplace=True)

        self.qkv_conv = nn.Conv2d(self.dim,2 * self.dim + self.dim,kernel_size=(1, stride),stride=(1, stride))

        self.attn_out = nn.Conv2d(self.dim, self.dim, kernel_size=1)
        #frames = {48: 300, 96: 150, 192: 75}.get(out_channels, 150)
        frames = {48: 10, 96: 10, 192: 10}.get(self.out_dim, 150)
        self.key_rel = nn.Parameter(torch.randn(2 * 10 - 1, self.dk // self.heads))
    ...
    def forward(self, x,T=1):
        ...
        flat_q, flat_k, flat_v = self.compute_flat_qkv(x)
        B, _, _, T = flat_q.shape

        logits = torch.matmul(flat_q.transpose(2, 3), flat_k)
        rel_logits = self.relative_logits(flat_q.shape, q=flat_q)
        weights = F.softmax(logits + rel_logits, dim=-1)

        #Apply Bernoulli dropout mask
        mask = torch.bernoulli(torch.full((B * self.heads * T,), 0.5, device=device)).reshape(B, self.heads, T, 1)
        weights = weights * mask.expand(-1, -1, -1, T)
        weights = weights / (weights.sum(dim=3, keepdim=True) + 1e-8)

        attn_out = torch.matmul(weights, flat_v.transpose(2, 3)).reshape(B, self.heads, 1, T, self.dim // self.heads)
        attn_out = attn_out.permute(0, 1, 2, 4, 3)
    ...
    def compute_flat_qkv(self, x):
        ...
        dkh = self.dim // self.heads
        q = q * (dkh ** -0.5)

        flat_q = q.reshape(N, self.heads, dkh, V1 * T1)
        flat_k = k.reshape(N, self.heads, dkh, V1 * T1)
        flat_v = v.reshape(N, self.heads, self.dim // self.heads, V1 * T1)

        return flat_q, flat_k, flat_v

    def split_heads_2d(self, x):
        B, channels, F, V = x.shape
        return x.reshape(B, self.heads, channels // self.heads, F, V)

    def combine_heads_2d(self, x):
        B, Nh, dv, F, V = x.shape
        return x.reshape(B, Nh * dv, F, V)

    def relative_logits(self, shape, q):
        B, Nh, dk, _, T = shape[0], shape[1], shape[2], shape[3], shape[3]
        q = q.permute(0, 1, 3, 4, 2).reshape(B, Nh, T, dk)
        rel_logits = torch.einsum('bhld,md->bhlm', q, self.key_rel)
        return self.rel_to_abs(rel_logits)

    def rel_to_abs(self, x):
        B, Nh, L, _ = x.shape
        x = torch.cat([x, torch.zeros(B, Nh, L, 1, device=x.device)], dim=3)
        flat_x = x.reshape(B, Nh, L * 2 * L)
        flat_x = torch.cat([flat_x, torch.zeros(B, Nh, L - 1, device=x.device)], dim=2)
        return flat_x.reshape(B, Nh, L + 1, 2 * L - 1)[:, :, :L, L - 1:]
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask = None,T=1):
        for attention, mlp in self.layers:
            x = attention(x, mask = mask,T=T) # go to attention
...
class TCN_TTRANSF_unit(nn.Module):
    def __init__(self, in_channels, out_channels, heads=3, stride=1, residual=True, dropout=0.1, mask=None, mask_grad=True):
        ...
        self.transf1 = Transformer(dim=in_channels, dim_out = out_channels, depth=1, heads=heads, mlp_dim=in_channels, dropout=dropout)
