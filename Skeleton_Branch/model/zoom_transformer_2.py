...
class TemporalTransformer(nn.Module):
    def __init__(self, in_channels, heads=3, dropout=0.1, frames=64, relative=True):
        super(TemporalTransformer, self).__init__()
        self.in_channels = in_channels
        self.heads = heads
        self.dk = in_channels // heads  # Key/query dimension per head
        self.dv = in_channels // heads  # Value dimension per head
        self.relative = relative
        self.frames = frames

        # Q, K, V projection
        self.qkv_conv = nn.Conv2d(in_channels, 3 * in_channels, kernel_size=(1, 1), stride=(1, 1))
        ...
        if self.relative:
            self.key_rel = nn.Parameter(
                torch.randn((2 * frames - 1, self.dk), requires_grad=True)
            )
            nn.init.normal_(self.key_rel, 0, 0.02)
    ...
    def forward(self, x):
        ...
        # Reshape to process temporal dimension: (N*V, C, T, 1)
        x = x.permute(0, 3, 1, 2).contiguous().view(N * V, C, T, 1)

        # Compute Q, K, V
        qkv = self.qkv_conv(x)  # (N*V, 3*C, T, 1)
        q, k, v = torch.split(qkv, [self.in_channels, self.in_channels, self.in_channels], dim=1)

        # Split into heads: (N*V, heads, C/heads, T, 1)
        q = rearrange(q, 'b (h d) t v -> b h d t v', h=self.heads)
        k = rearrange(k, 'b (h d) t v -> b h d t v', h=self.heads)
        v = rearrange(v, 'b (h d) t v -> b h d t v', h=self.heads)

        # Compute attention scores
        scores = torch.einsum('bhdti,bhdki->bhtk', q, k) * self.scale  # (N*V, heads, T, T)

        # Relative positional encodings
        if self.relative:
            rel_scores = self.relative_logits(q)  # (N*V, heads, T, T)
        ...
        attn_out = torch.einsum('bhtk,bhdki->bhdti', weights, v)  # (N*V, heads, C/heads, T, 1)
        attn_out = rearrange(attn_out, 'b h d t v -> b (h d) t v')  # (N*V, C, T, 1)

        # Output projection
        attn_out = self.out_conv(attn_out)  # (N*V, C, T, 1)

        # Reshape back to (N, C, T, V)
        attn_out = attn_out.view(N, V, C, T).permute(0, 2, 3, 1).contiguous()

        return attn_out

    def relative_logits(self, q):
        B, Nh, dk, T, V = q.size()
        q = q.permute(0, 1, 4, 3, 2).contiguous().view(B, Nh, T, dk)  # (N*V, heads, T, dk)
        rel_logits = torch.einsum('bhtd,ld->bhtl', q, self.key_rel)  # (N*V, heads, T, 2*T-1)
...
class TCN_STRANSF_unit(nn.Module):
    def __init__(self, in_channels, out_channels, heads=3, stride=1, residual=True, dropout=0.1, mask=None, mask_grad=True, frames=64):
        ...
        self.temp_transf = TemporalTransformer(in_channels=in_channels, heads=heads, dropout=dropout, frames=frames)
        self.out_channels = out_channels
        self.combine_conv = nn.Conv2d(out_channels + in_channels, out_channels, kernel_size=(1, 1))
        nn.init.xavier_uniform_(self.combine_conv.weight)
        nn.init.zeros_(self.combine_conv.bias)
        self.relu = nn.ReLU()
        if not residual:
            self.residual = lambda x: 0
        elif (in_channels == out_channels) and (stride == 1):
    ...
    def forward(self, x, mask=None):
        ...
        tx = self.transf1(tx, mask=self.mask if mask is None else mask)
        tx = tx.view(B, T, V, C).permute(0, 3, 1, 2).contiguous()
        # Temporal Transformer branch
        temp_tx = self.temp_transf(x)
        # TCN branch
        tcn_x = self.tcn1(tx)
        # Combine spatial transformer and TCN outputs
        combined_x = torch.cat([tcn_x, temp_tx], dim=1)  # (B, out_channels + in_channels, T, V)
        x = self.combine_conv(combined_x)  # (B, out_channels, T, V)
        x = x + self.residual(x)
        return self.relu(x)
